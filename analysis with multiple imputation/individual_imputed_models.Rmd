---
title: "Individual Imputed Models"
output:
  word_document:
    toc: yes
    toc_depth: '3'
...
### ANOVA summary
```{r}
Anova(mod_mice_imp1, type = 3) %>%
  knitr::kable(digits = c(2, 2, 2, 3))
```

### Effect Size
```{r}
etaSquared(mod_mice_imp1, type = 3, anova = TRUE) %>% 
  knitr::kable(digits = 3)
```
...
```{r}
# storing residuals
store_residuals <- augment(mod_mice_imp1)

# plotting histogram of residuals
ggplot(data = store_residuals, aes(x = .resid)) + 
  geom_density(fill = "purple") + 
  stat_function(linetype = 2, 
                fun      = dnorm, 
                args     = list(mean = mean(store_residuals$.resid), 
                                sd   =   sd(store_residuals$.resid))) +
  theme_minimal()


# QQ-plot
ggplot(mod_mice_imp1) +
...
The spread of the residuals appears to be approximately the same across the range of fitted values.
...
ols_plot_resid_stand(mod_mice_imp1)


# or grabbing from model augment output
model_aug <- augment(mod_mice_imp1)

model_aug$id <- mod_mice_imp1$.rownames

std_resids <- model_aug %>% 
...
```{r}
# using olsrr function
ols_plot_cooksd_chart(mod_mice_imp1)
...
### ANOVA summary
```{r}
Anova(mod_mice_imp2, type = 3) %>%
  knitr::kable(digits = c(2, 2, 2, 3))
```

### Effect Size
```{r}
etaSquared(mod_mice_imp2, type = 3, anova = TRUE) %>% 
  knitr::kable(digits = 3)
```
...
```{r}
# storing residuals
store_residuals <- augment(mod_mice_imp2)
...
```{r}
# using olsrr function
ols_plot_resid_stand(mod_mice_imp2)


# or grabbing from model augment output
model_aug <- augment(mod_mice_imp2)

model_aug$id <- mod_mice_imp2$.rownames
...
```{r}
# using olsrr function
ols_plot_cooksd_chart(mod_mice_imp2)
...
### ANOVA summary
```{r}
Anova(mod_mice_imp3, type = 3) %>%
  knitr::kable(digits = c(2, 2, 2, 3))
```

### Effect Size
```{r}
etaSquared(mod_mice_imp3, type = 3, anova = TRUE) %>% 
  knitr::kable(digits = 3)
```
...
```{r}
# storing residuals
store_residuals <- augment(mod_mice_imp3)
...
```{r}
# using olsrr function
ols_plot_resid_stand(mod_mice_imp3)


# or grabbing from model augment output
model_aug <- augment(mod_mice_imp3)

model_aug$id <- mod_mice_imp3$.rownames
...
```{r}
# using olsrr function
ols_plot_cooksd_chart(mod_mice_imp3)
...
### ANOVA summary
```{r}
Anova(mod_mice_imp4, type = 3) %>%
  knitr::kable(digits = c(2, 2, 2, 3))
```

### Effect Size
```{r}
etaSquared(mod_mice_imp4, type = 3, anova = TRUE) %>% 
  knitr::kable(digits = 3)
```
...
```{r}
# storing residuals
store_residuals <- augment(mod_mice_imp4)
...
```{r}
# using olsrr function
ols_plot_resid_stand(mod_mice_imp4)


# or grabbing from model augment output
model_aug <- augment(mod_mice_imp4)
...
```{r}
# using olsrr function
ols_plot_cooksd_chart(mod_mice_imp4)
...
### ANOVA summary
```{r}
Anova(mod_mice_imp5, type = 3) %>%
  knitr::kable(digits = c(2, 2, 2, 3))
```

### Effect Size
```{r}
etaSquared(mod_mice_imp5, type = 3, anova = TRUE) %>% 
  knitr::kable(digits = 3)
```
...
```{r}
# storing residuals
store_residuals <- augment(mod_mice_imp5)
...
```{r}
# using olsrr function
ols_plot_resid_stand(mod_mice_imp5)


# or grabbing from model augment output
model_aug <- augment(mod_mice_imp5)

model_aug$id <- mod_mice_imp5$.rownames
...
```{r}
# using olsrr function
ols_plot_cooksd_chart(mod_mice_imp5)
...
Comparing imputation models (ANOVA output):
```{r}
Anova(mod_mice_imp1, type = 3) %>%
...
```{r}
data_imp1 %>%
  dplyr::select(biospheric, altruistic, egoistic, hedonic, ingroup_identification, clothing_interest, self_deceptive_sdr, impress_manag_sdr, Age, consumer_intentions) %>%
  cor() %>%
  knitr::kable(digits = 2)
...
Combining Correlations across Imputed Data Sets
```{r}
micombine.cor(mids_obj, variables = c("biospheric", "altruistic", "egoistic", "hedonic", "ingroup_identification", "clothing_interest", "self_deceptive_sdr", "impress_manag_sdr", "Age", "consumer_intentions"), method = "pearson") %>%
  knitr::kable(digits = 2)
```
