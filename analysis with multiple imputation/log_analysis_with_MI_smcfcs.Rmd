...
```{r}
D2(log_mice, log_minus_frameXnorm)
```
...
```{r}
D2(log_mice, log_minus_frameXbio)
```

Note: I think df1=10 because it's also removing the higher-order interactions involving biospheric values. Not sure why this isn't an issue for the NormXFrame interaction, which is also involved in higher-order interactions in the model.

- FrameXBio = 2 codes
- FrameXBioXNorm = 8 codes
...
```{r}
D2(log_mice, log_minus_frameXalt)
```
...
```{r}
D2(log_mice, log_minus_frameXego)
```
...
```{r}
D2(log_mice, log_minus_frameXhed)
```
...
```{r}
D2(log_mice, log_minus_frameXingroup)
```
...
```{r}
D2(log_mice, log_minus_normXbio)
```
...
```{r}
D2(log_mice, log_minus_normXalt)
```
...
```{r}
D2(log_mice, log_minus_normXego)
```
...
```{r}
D2(log_mice, log_minus_normXhed)
```
...
```{r}
D2(log_mice, log_minus_normXingroup)
```
...
```{r}
D2(log_mice, log_minus_frameXnormXbio)
```
...
```{r}
D2(log_mice, log_minus_frameXnormXalt)
```
...
```{r}
D2(log_mice, log_minus_frameXnormXego)
```
...
```{r}
D2(log_mice, log_minus_frameXnormXhed)
```
...
```{r}
D2(log_mice, log_minus_frameXnormXingroup)
```
...
emmip(log_mice, ~ framing_condition, type = "response", CIs = TRUE, CIarg = list(lwd = 2, alpha = 0.5), xlab = "Framing Condition", ylab = "Consumer Intentions") + scale_x_discrete(breaks=c("control_framing","pro_env_framing","self_enh_framing"),
...
emmip(log_mice, ~ norm_condition, type = "response", CIs = TRUE, CIarg = list(lwd = 2, alpha = 0.5), xlab = "Norm Condition", ylab = "Consumer Intentions") + scale_x_discrete(breaks=c("control_norm","descriptive_norm", "convention_norm", "social_norm", "moral_norm"),
...
emmip(log_mice, framing_condition ~ norm_condition, type = "response", CIs = TRUE, CIarg = list(lwd = 2, alpha = 0.5), xlab = "Norm Condition", ylab = "Consumer Intentions") + scale_x_discrete(breaks=c("control_norm","descriptive_norm", "convention_norm", "social_norm", "moral_norm"),
...
# logit scale
FxN_comparisons <- contrast(cell_emmeans_logit, 
...
# probability scale
FxN_comparisons <- contrast(cell_emmeans_prob, 
                          method = list("Control Frame + DN - Control Frame + Ctrl" = cf_dn_emm - cf_cn_emm,
                                        "Control Frame + Conv - Control Frame + Ctrl" = cf_conv_emm - cf_cn_emm,
                                        "Control Frame + SN - Control Frame + Ctrl" = cf_sn_emm - cf_cn_emm,
                                        "Control Frame + MN - Control Frame + Ctrl" = cf_mn_emm - cf_cn_emm,
